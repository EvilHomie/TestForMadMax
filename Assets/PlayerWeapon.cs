...
public class PlayerWeapon : MonoBehaviour, IWeapon, IItem
{
    ...
    IEnumerator SingleBarreledShoot()
    {
        while (_isShooting) 
        {
            ...
            {                
                _firePoints[0].OneShoot(_weaponData.shootSound);
                CameraManager.Instance.Shake(1f / CurFireRate, _weaponData.shakeOnShootIntensity);
                if (Physics.Raycast(transform.position, transform.forward, out RaycastHit hitInfo))
                {
                    hitInfo.collider.GetComponent<IDamageable>()?.OnHit(CurHullDmg, CurShieldDmg, _weaponData.hitSound);
                }
    ...
    {        
        while (_isShooting)
        {
            if (Time.time >= _nextTimeTofire)
            {
                ...
                _firePoints[_lastShootBarrelNumber].OneShoot(_weaponData.shootSound);
                CameraManager.Instance.Shake(1f / CurFireRate, _weaponData.shakeOnShootIntensity);
                if (Physics.Raycast(transform.position, transform.forward, out RaycastHit hitInfo))
                {
                    hitInfo.collider.GetComponent<IDamageable>()?.OnHit(CurHullDmg, CurShieldDmg, _weaponData.hitSound);
